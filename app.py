...
def init_all_connections():
    ...
    # os.environ.get()과 st.secrets.get()을 사용하여 에러 발생을 원천 차단합니다.
    ...
    if firebase_json:
        try:
            # 문자열 형태라면 JSON으로 파싱, 이미 딕셔너리라면 그대로 사용
            ...
            st.error(f"Firebase 인증 데이터 파싱 실패: {e}")
    ...
    if creds is None:
        st.error("보안 키를 찾을 수 없습니다. Cloud Run의 환경 변수 또는 Secret Manager 설정을 확인해주세요.")
        st.stop()
...
# --- 6. [화면 3] 시스템 상태 ---
    ...
    st.header("⚙️ 시스템 상태")
    col1, col2 = st.columns(2)
    with col1: st.metric("DB Status", "Connected", "Supabase")
    with col2: st.metric("Storage Status", "Connected", "Firebase")
